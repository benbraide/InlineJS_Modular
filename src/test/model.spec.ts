...
describe('x-model directive', () => {
    ...
    it('should not reflect data changed elsewhere with the \'.oneway\' modifier', async () => {
        document.body.innerHTML = `
            <div x-data="{ foo: '' }">
        ...
        await waitFor(() => { expect(document.querySelector('span').textContent).equal('out text') });
    });
    ...
    it('should cast value to number if \'.number\' modifier is present', async () => {
        ...
        await waitFor(() => { expect(Region.Find(key, true)['foo']).equal(123) });
    });
    ...
    it('should cast to null if empty, original value if casting fails, numeric value if casting passes', async () => {
        ...
        await waitFor(() => { expect(Region.Find(key, true)['foo']).equal(null) });
        ...
        await waitFor(() => { expect(Region.Find(key, true)['bar']).equal('') });
        ...
        await waitFor(() => { expect(Region.Find(key, true)['bar']).equal(-123) });
    });
