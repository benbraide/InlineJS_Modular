...
exports.DirectiveHandler = void 0;
...
class DirectiveHandler {
    ...
    static CreateProxy(getter, contains, setter, target) {
        return region_1.Region.CreateProxy(getter, contains, setter, target);
    }
    ...
    static DoEvaluation(region, element, expression, ignoreRemoved, useBlock, ctxName, ctx, useWindow = false) {
        ...
        try {
            ...
            result = ((result instanceof proxy_1.Value) ? result.Get() : result);
        }
        catch (err) {
    ...
    static ToString(value) {
        return region_1.Region.ToString(value);
    }
    ...
    static GetOptions(options, specifiedOptions, handler, needsOptions = false) {
        specifiedOptions.forEach((option, index, list) => {
            if ((!handler || (needsOptions ? !handler(options, option, index, list) : !handler(option, index, list))) && option in options && typeof options[option] === 'boolean') {
                options[option] = true;
            }
        });
        return options;
...
exports.DirectiveHandler = DirectiveHandler;
