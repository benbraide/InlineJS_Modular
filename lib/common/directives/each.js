...
class EachDirectiveHandler extends generic_1.DirectiveHandler {
    constructor() {
        super('each', (region, element, directive) => {
            ...
            }, region_1.Region.GetConfig().GetDirectiveName(this.key_)), isCount = false, isReverse = false;
            if (!info) {
            ...
            let arrayChangeHandler = (myRegion, change, isOriginal) => {
                let removeRange = (myRegion, index, count) => {
                    ...
                    let max = (options.clones.length - getTarget(generic_1.DirectiveHandler.Evaluate(myRegion, element, expression)).length);
                    if (max <= 0) { //Nothing to remove
                        return false;
                    }
                    count = ((max < count) ? max : count);
                    options.clones.splice(index, count).forEach(myInfo => control_1.ControlHelper.RemoveItem(myInfo.itemInfo, info));
                    return count;
                };
                if (isOriginal) {
