    ...
    static Bind(region, element, directive, isHtml, callback, allowAnimation) {
        ...
        region.GetState().TrapGetAccess(() => {
            if (!callback || callback()) {
                let value = generic_1.DirectiveHandler.Evaluate(region_1.Region.Get(regionId), element, directive.value);
                if (firstEntry || !region_1.Region.IsEqual(value, lastValue)) {
                    if (active) {
