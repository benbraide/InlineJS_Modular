...
exports.RouterGlobalHandler = exports.RouterDirectiveHandler = exports.BackPath = void 0;
    ...
    constructor(router, mountInfo, modal = null) {
        super(router.GetKey(), (region, element, directive) => {
            ...
            if (directive.arg.key === 'register') {
                ...
                if (directive.value !== region_1.Region.GetConfig().GetDirectiveName(this.key_)) {
                ...
                let regionId = region.GetId(), setState = (key, value) => {
                    if (key in state && !region_1.Region.IsEqual(state[key], value)) {
                        state[key] = value;
                        region_1.Region.Get(regionId).GetChanges().AddComposed(key, `${mountInfo.scopeId}.mount`);
                    }
                };
                this.fetch_ = new fetch_1.Fetch(null, mountInfo.element, {
                    onBeforeRequest: () => {
                    ...
                    onLoad: () => {
                        ...
                        (new bootstrap_1.Bootstrap()).Attach(mountInfo.element);
                ...
                mountInfo.proxy = region_1.Region.CreateProxy((prop) => {
                    if (prop in state) {
                        region_1.Region.Get(regionId).GetChanges().AddGetAccess(`${mountInfo.scopeId}.mount.${prop}`);
                        return state[prop];
                    }
                    if (prop === 'element') {
...
class RouterGlobalHandler extends generic_2.GlobalHandler {
    constructor(middlewares_ = new Array(), modal = null, ajaxPrefix_ = 'ajax', mountElementType = '') {
        super('router', null, null, () => {
            ...
            region_1.Region.GetDirectiveManager().AddHandler(new RouterDirectiveHandler(this, this.mountInfo_, modal));
